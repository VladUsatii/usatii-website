...
$$
\text{attention captured} \;\rightarrow\; \text{attention sustained} \;\rightarrow\; \text{attention recycled}
$$
...
We can define $$A$$ as a union of novelty, coherence, and relation over each sentence, awarding a score from $$[0..1]$$ for each sentence. Redundancy, defined as $$S = A / sec$$, should stay consistent across the full video. If it dips towards $$0$$, you're sacrificing millions of potential views (which means you're sacrificing revenue).
...
> **Weak opening:** "I want to talk about what I’ve learned from building."
> **Stronger opening:** "Founders, stop making nerdy content. You aren't talking to your audience right -- 5 ways to make good founder content, let's go:"
...
> "Most founder content is boring.
> And most people blame the algorithm instead of their script.
> You should be blaming that LLM!"
...
$$ I_{t+1} = I_t \cdot \left(1 + \alpha h + \beta r + \gamma e + \delta \ell \right) $$
...
h = \text{hook strength},
\quad
r = \text{retention quality},
\quad
e = \text{engagement quality},
\quad
\ell = \text{loop / rewatch effect}.
$$
...
An internal scoring model -- a good management heuristic -- we use: $$ Q = 0.35H + 0.30C + 0.20S + 0.15L $$ where

$$
H = \text{hook hold rate}, \quad
C = \text{completion rate}, \quad
S = \text{share/save/comment quality}, \quad
L = \text{loop-induced rewatches}.
$$
...
If you want similar results, reach out to _vlad@usatii.com_.